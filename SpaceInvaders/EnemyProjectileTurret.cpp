...
EnemyProjectileTurret::EnemyProjectileTurret()
{
...
EnemyProjectileTurret::EnemyProjectileTurret(unsigned newHandle, int amountOfFrames, Vector2 newPosition)
{
...
void EnemyProjectileTurret::Movement(GlobalData* gd)
{
...
void EnemyProjectileTurret::Shoot(GlobalData* gd)
{
	...
	{
		//Spawn bullet
		EnemyProjectile newProjectile((gd)->EnemyProjectileTexture, 1, Vector2(position.x + forwardDirection.x, position.y + forwardDirection.y), 150, textureAngle, target);
		newProjectile.SetCollider(10, 10);
		(gd)->EnemyProjectiles.push_back(newProjectile); //Adds to list

		shootTimer = 0;
	}
}

void EnemyProjectileTurret::OnUpdateLogic(GlobalData* gd)
{
	if (runOnce == true)
	{
...
void EnemyProjectileTurret::ResetAll()
{
}
