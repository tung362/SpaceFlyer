...
EnemyMissile::EnemyMissile(unsigned newHandle, int amountOfFrames, Vector2 newPosition, float newSpeed, float newAngle, PlayerMode2 * newTarget)
{
...
void EnemyMissile::Destroy()
{
	destroyed = true;
}

bool EnemyMissile::isDestroyed()
{
	return destroyed;
}

bool EnemyMissile::GetCanDamage()
{
	return canDamage;
}

void EnemyMissile::SetCanDamage(bool newCanDamage)
{
	canDamage = newCanDamage;
}

ColliderBounds EnemyMissile::GetCollider()
{
	return collider;
}

void EnemyMissile::Movement(GlobalData* gd)
{
	if (runOnce == true)
	{
...
template<typename T>
bool EnemyMissile::CollisionCheck(T & other)
{
