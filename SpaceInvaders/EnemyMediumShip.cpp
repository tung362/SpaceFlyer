...
EnemyMediumShip::EnemyMediumShip()
{
...
EnemyMediumShip::EnemyMediumShip(unsigned newHandle, int amountOfFrames, Vector2 newPosition, float newSpeed, GlobalData* gd)
{
	theTexture = newHandle;
	maxFrame = amountOfFrames - 1;
	position = newPosition;
	SetTextureSettings(100.0f, 100.0f, 0, true);
	speed = newSpeed;
	rotationSpeed = 1.2f;
...
void EnemyMediumShip::CreateShipParts(GlobalData* gd)
{
	//Blasters
	EnemyBlasterTurret enemyBlasterTurret1((gd)->EnemyBlasterTurretTexture, 1, Vector2(position.x + 20, position.y + 4));
	Blasters.push_back(enemyBlasterTurret1);

	EnemyBlasterTurret enemyBlasterTurret2((gd)->EnemyBlasterTurretTexture, 1, Vector2(position.x + 20, position.y - 4));
	Blasters.push_back(enemyBlasterTurret2);

	//Missiles
	EnemyMissileTurret enemyMissileTurret1((gd)->EnemyMissleTurretTexture, 1, Vector2(position.x - 10, position.y + 18));
	Missiles.push_back(enemyMissileTurret1);

	EnemyMissileTurret enemyMissileTurret2((gd)->EnemyMissleTurretTexture, 1, Vector2(position.x - 10, position.y - 18));
	Missiles.push_back(enemyMissileTurret2);

	//AntiProjectiles
	EnemyAntiProjectileTurret enemyAntiProjectileTurret1((gd)->EnemyAntiProjectileTurretTexture, 1, Vector2(position.x + 5, position.y));
...
void EnemyMediumShip::CalculateDirection(GlobalData* gd)
{
...
void EnemyMediumShip::Destroy()
{
	destroyed = true;
}

bool EnemyMediumShip::isDestroyed()
{
	return destroyed;
}

void EnemyMediumShip::Movement(GlobalData* gd)
{
	//Rotation
	...
	if (targetDistance > 400) movement = forwardDirection;

	//Apply movement
	position += (movement * speed) * getDeltaTime();
}

void EnemyMediumShip::OnUpdateLogic(GlobalData* gd)
{
	...
	if (health <= 0)
	{
		...
		{
			int chance;
			chance = Randint(1, 7); //71% chance of anything dropping
...
template<typename T>
bool EnemyMediumShip::CollisionCheck(T & other)
{
	double radians = textureAngle * (PI / 180);
	double degrees = 180 * radians / PI;
	float colliderAngle;
	...
	//Gets EnemyMediumShip collider vertex (Position depends on the angle)
	Vector2 TopLeft = RotatePoint(position.x + colliderOffset.x, position.y + colliderOffset.y, colliderAngle, Vector2((position.x - collider.width / 2) + colliderOffset.x, (position.y + collider.height / 2) + colliderOffset.y)); //Top Left
	Vector2 BottomRight = RotatePoint(position.x + colliderOffset.x, position.y + colliderOffset.y, colliderAngle, Vector2((position.x + collider.width / 2) + colliderOffset.x, (position.y - collider.height / 2) + colliderOffset.y)); //Bottom Right
	Vector2 TopRight = RotatePoint(position.x + colliderOffset.x, position.y + colliderOffset.y, colliderAngle, Vector2((position.x + collider.width / 2) + colliderOffset.x, (position.y + collider.height / 2) + colliderOffset.y)); //Top Right
	Vector2 BottomLeft = RotatePoint(position.x + colliderOffset.x, position.y + colliderOffset.y, colliderAngle, Vector2((position.x - collider.width / 2) + colliderOffset.x, (position.y - collider.height / 2) + colliderOffset.y)); //Bottom Left
